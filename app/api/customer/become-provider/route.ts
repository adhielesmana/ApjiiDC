...
export async function POST(req: Request) {
  try {
    ...
    // Handle the logo file separately - like the space route handles images
    ...
    console.log("Sending to backend:", {
      url: `${BACKEND_URL}/provider/new`,
      name: backendFormData.get("name"),
      email: backendFormData.get("contact.email"),
      phone: backendFormData.get("contact.phone"),
    });

    // Create a clean token for the header, matching the space route exactly
    const authToken = token.value.startsWith("Bearer ")
      ? token.value
      : `Bearer ${token.value}`;

    // Make the request with exactly the same structure as the space route
    const response = await axios.post(
      `${BACKEND_URL}/provider/new`,
      backendFormData,
      {
        headers: {
